...
struct NetworkManager {
  ...
  - Parameters:
      ...
      - error: error message
  */
  func fetchPost(detail: Parameters, completion: @escaping (_ feeds: [FeedResult]?, _ error: String?) -> Void) {

    router.request(.getPosts(detail: detail)) { data, response, error in
      if error != nil {
      ...
      if let response = response as? HTTPURLResponse {
        ...
        case .success:
          ...
          do {
            ...
            let apiResponse = try JSONDecoder().decode([FeedResult].self, from: newData)
